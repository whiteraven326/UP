...
class TennisSolo(BaseTennis):
    ...
    def start(self, num=None):
        if num is None:
            ...
            if not self.pause:
                self.remaining_time = num - 1
                self.c.itemconfig(self.countdown_text, text=str(num))
                self.root.after(1000, self.start, self.remaining_time)
            else:
                self.remaining_time = num
        else:
        ...
        if key in {"w", "s"}:
            ...
            if event.keysym == "q":
                ...
                if self.pause:
                    ...
                    self.c.create_text(450, 270, text="Игра на паузе", font=("Comic Sans MS", 75, "bold"), fill="red",

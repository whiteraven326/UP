...
class Menu:
    def __init__(self, master):
        ...
        master.protocol("WM_DELETE_WINDOW", self.on_closing)  # Добавлено
...
class BaseTennis:
    ...
    def end_game(self, message):
        ...
        self.c.create_text(300, 180, text=message, font=("Comic Sans MS", 55, "bold"), fill="black")
        self.c.pack()


class TennisSolo(BaseTennis):
    def __init__(self, root):
        super().__init__(root, "Теннис_Соло", "ten.png")
    ...
    def score(self, player):
        if player == "right":
            ...
            if self.bl_score == 5:
                self.game_over_image = PhotoImage(file="res.png")
                self.end_game("ВЫ\nПРОИГРАЛИ!")
        else:
            self.red_score += 1
...
class Tennis(BaseTennis):
    def __init__(self, root):
        super().__init__(root, "Теннис_Вдвоём", "ten.png")
        self.countdown(self.countdown_time)
        self.c.bind("<KeyPress>", self.movement_handler)
        self.c.bind("<KeyRelease>", self.stop_pad)
